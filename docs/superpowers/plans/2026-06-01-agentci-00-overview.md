...
| D8 | **Promotion bar** | A fix **qualifies** iff: held-out mean correctness lift ≥ `MIN_HELDOUT_LIFT` over baseline **AND** zero held-out cases flip pass→fail. A non-qualifying fix keeps the gate **RED** ("regression confirmed, no qualifying fix"). Qualifying only makes a fix *eligible* — actual promotion is human-approved (D12). **Amended 2026-06-09 (recovery-correct criterion):** `MIN_HELDOUT_LIFT` lowered `+0.05 → 0.0`. A fix is a regression *recovery* — its goal is to restore baseline, so its realistic best is parity (lift ~0), not a strict improvement. The original `+0.05` suited the curated demo but structurally reds every real recovery (a fix that merely restores baseline scores ~0). The safe-to-ship test is "no worse than baseline (lift ≥ 0) AND no new held-out regressions" — which is what the gate now encodes. The zero-regression leg is unchanged. **Amended 2026-06-10 (noise-calibrated flip leg):** the flip leg is measured against the sampling-noise floor, not zero. Evidence (`scripts/calibrate_gate.py`): re-sampling the *unchanged baseline prompt* on held-out and gating it against its own recorded answers gives lift 0.0 but **2** pass→fail flips (temp-0 on Vertex is not reproducible; judges wobble ±0.1–0.2; knife-edge cases sit at exactly 0.70) — a no-op change cannot pass a zero-flip gate. `MAX_HELDOUT_REGRESSIONS` is therefore set to the measured floor (currently 2/16) and must be re-measured whenever target/judge/ruler models change. The gate's guarantee becomes "no more held-out regressions than re-running production unchanged produces". |
...
| D17 | **Frozen cross-family improvement ruler** | Held-out correctness lift (D8) is scored by `IMPROVEMENT_JUDGE_MODEL`, a non-Gemini family distinct from `ENGINEER_MODEL`. The agent cannot grade its own homework. **Amended 2026-06-05:** the ruler transport is env-selected — when `GROQ_API_KEY` is set, the ruler runs `FREE_RULER_MODEL` (Llama on Groq's free tier; still non-Gemini, so the independence constraint holds) instead of paid Anthropic. Same single choke point (`_independent_json`), same cache keys. **Amended 2026-06-09:** when NO non-Gemini endpoint has quota (Claude-on-Vertex defaults to ~0 on fresh projects), `AGENTCI_RULER=gemini` routes the ruler to `RULER_GEMINI_MODEL = gemini-2.5-flash-lite` — a distinct Gemini checkpoint from `ENGINEER_MODEL` (different weights, separate quota pool), so the fix-author never scores its own output with its own weights. Checkpoint-level separation is weaker than cross-family and remains the documented fallback, not the default. |
